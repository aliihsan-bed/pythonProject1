...
# Kamera ve arka plan görüntüsü ayarları
cap = cv2.VideoCapture(0)
cap.set(3, 640)
cap.set(4, 480)
imgBackground = cv2.imread('Resources/background.png')

# Mod görüntülerini yükleme
folderModePath = 'Resources/Modes'
modePathList = os.listdir(folderModePath)
imgModeList = [cv2.imread(os.path.join(folderModePath, path)) for path in modePathList]

# Kodlama dosyasını yükleme
...
while True:
    ...
    img = cv2.flip(img, 1)  # Yatay olarak çevirme
    ...
    if faceCurFrame:
        ...
        if counter != 0:
            if counter == 1:
                ...
                # Görüntüyü Firebase Storage'dan çekme
                blob = bucket.blob(f'Images/{id}.jpg')
                if blob.exists():
                    array = np.frombuffer(blob.download_as_string(), np.uint8)
                    imgStudent = cv2.imdecode(array, cv2.IMREAD_COLOR)
                else:
                    print(f"Görüntü {id} bulunamadı")
    ...
    # Programı sonlandırmak için 'q' tuşuna basmayı kontrol et
